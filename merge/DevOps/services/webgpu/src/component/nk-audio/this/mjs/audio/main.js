...
// Create an atomic state for synchronization between Worker and AudioWorklet.
  ...
  const initializeAudio = async () => {
    ...
    const processor = new URL('./basic-processor.js', import.meta.url)
    await audioContext.audioWorklet.addModule(processor.pathname);
    ...
    oscillatorNode.connect(gainNode).connect(processorNode).connect(audioContext.destination);
...
// Detect required features.
  ...
  const audioWorker = new URL('./worker.js', import.meta.url)
  // Create a WebWorker for Audio Processing.
  worker = new Worker(audioWorker.pathname, {
    name: 'audio',
    type: 'module'
  });
