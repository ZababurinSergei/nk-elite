...
export default async function http(url, request = {}) {
  ...
  try {
    ...
    if (!res) {
      ...
      // errorDialog(`В данный момент система недоступна.<br> Попробуйте подключиться <br>через несколько минут.`)
    ...
    console.log('!!!!!!!!!!!! НАДО ЗАМЕНИТЬ НА ОТВЕТ С ОШИБКОЙ !!!!!!!!!!!!', error);
    throw error;
  }

  if (!res.ok) {
    // console.log('dddddddddddddddddddddddddddddddddddddddddddddddddd', res)
    // const error = new Error(res.statusText || `response status is ${res.status}`);
    // error.status = res.status;
    // error.statusCode = res.status;
    // error.response = res;
    res.body = {
      code: res.status,
      description: res.statusText || `response status is ${res.status}`,
    };
    return res;
    // throw error;
  }
