export const OSC = async () => {
    ...
    const osc_property = async () => {
        ...
        for (let i = 0; i < 8; i++){
            state.osc_out[i].line = state.osc_lp[i].i_s * state.osc_substrate_s.step[i] * state.osc_p[i].norm;
            state.osc_out[i].line_sin = square_sin(state.osc_out[i].line, state.osc_lp[i].i_ph, state.osc_p[i].up_lim * state.osc_p[i].norm);
    ...
    const init = () => {
        ...
        for (let i = 0; i < 8; i++) {
            ...
            state.osc_p[i].norm = 1 / state.osc_p[i].up_lim;
