 ...
 */
export function cloneInstance(origin) {
    try {
        ...
        return Object.assign(Object.assign({}, origin), prototypes);
    }
    catch (e) {
...
export function mergeFunctions(...funcs) {
    return function (...args) {
        funcs.filter((fn) => !!fn).forEach((fn) => fn === null || fn === void 0 ? void 0 : fn(...args));
    };
}
 ...
 */
export function flatObject(target) {
    const flatted = {};
    const recurse = (parent, target) => {
        for (const prop in target) {
            if (prop) {
 ...
 * @param target flat target
 */
export function normalizeFlattedObject(target) {
    let normalized = {};
    for (const prop in target) {
        if (prop) {
