...
import { getBuiltInColors } from './../../../this/services/theme/colorRegistry';
/**
 ...
 * @param colors
 */
export function convertToCSSVars(colors) {
    let cssVars = '';
...
function perfectColors(colors) {
    ...
    const inheritMap = [
        ...
        [
            ...
            'button.background',
            (color) => Color.fromHex(color).lighten(0.2),
        ],
    ];
    inheritMap.forEach(([inheritSourceColor, inheritTargetColor, transformer]) => {
        if (!nextColors[inheritSourceColor] &&
            nextColors[inheritTargetColor]) {
            if (transformer) {
                nextColors[inheritSourceColor] = transformer(nextColors[inheritTargetColor]);
            }
            else {
...
export function getThemeData(theme) {
    ...
    const updateRules = (s, token) => {
        ...
        if (index >= 0) {
            Object.assign(rules[index], Object.assign({}, token.settings));
        }
        else {
            if (s) {
                rules.push(Object.assign({ token: s }, token.settings));
            }
        }
    };
