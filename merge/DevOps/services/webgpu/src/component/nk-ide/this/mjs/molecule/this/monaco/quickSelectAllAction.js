import 'reflect-metadata';
import { container } from 'tsyringe';
import { localize } from './../../this/i18n/localize';
import { EditorService } from './../../this/services';
import { constants } from './../../this/services/builtinService/const';
import { KeyMod, KeyCode } from './../../this/monaco';
import { KeybindingWeight } from './../../this/monaco/common';
import { Action2 } from './../../this/monaco/action';
export class QuickSelectAllAction extends Action2 {
    constructor() {
        super({
            id: QuickSelectAllAction.ID,
            alias: QuickSelectAllAction.DESC,
            title: {
                value: QuickSelectAllAction.LABEL,
                original: QuickSelectAllAction.DESC,
            },
            f1: true,
            label: QuickSelectAllAction.LABEL,
            keybinding: {
                weight: KeybindingWeight.BuiltinExtension,
                when: undefined,
                primary: KeyMod.CtrlCmd | KeyCode.KeyA,
            },
        });
        this.editorService = container.resolve(EditorService);
    }
    selectEditorAll() {
        const editor = this.editorService.editorInstance;
        if (editor) {
            editor.focus();
            editor.setSelection(editor.getModel().getFullModelRange());
        }
    }
    isTextdom(ele) {
        return typeof ele.selectionStart === 'number';
    }
    run(accessor, ...args) {
        const focusinEle = args[0];
        // execute the action via shortcut if focusinEle is undefined
        const currentFocusinEle = focusinEle || document.activeElement;
        if (currentFocusinEle &&
            this.isTextdom(currentFocusinEle) &&
            !currentFocusinEle.className.includes('monaco')) {
            // native element can select by the native method
            currentFocusinEle.select();
        }
        else {
            // monaco component should use the method from instance
            this.selectEditorAll();
        }
    }
}
QuickSelectAllAction.ID = constants.ACTION_QUICK_SELECT_ALL;
QuickSelectAllAction.DESC = 'Select All';
QuickSelectAllAction.LABEL = localize('menu.selectAll', 'Select All');
