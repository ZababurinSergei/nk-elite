...
const BYTES_PER_SAMPLE = Float64Array.BYTES_PER_ELEMENT;
 ...
 * into an Array of Float64Array for the convinient WASM opearion.
 *
 * @class
...
class FreeQueue {
  ...
  constructor(wasmModule, length, channelCount, maxChannelCount) {
    ...
    for (let i = 0; i < this._channelCount; ++i) {
      this._channelDataLocal[i] = new Float64Array(length);
    }
  }

  /**
   * Allocates memory in the WASM heap and set up Float64Array views for the
   * channel data.
   *
   * @private
   */
  ...
  /**
   * Returns a Float64Array object for a given channel index. If the channel
  ...
  /**
   * Push a sequence of Float64Arrays to buffer.
   *
   * @param  {array} arraySequence A sequence of Float64Arrays.
   */
  push(arraySequence) {
  ...
  /**
   * Pull data out of buffer and fill a given sequence of Float64Arrays.
   *
   * @param  {array} arraySequence An array of Float64Arrays.
   */
  pull(arraySequence) {
    // The channel count of arraySequence and the length of each channel must
    // match with this buffer obejct.
