...
//Add Milky Way with a special magenta color
            ...
            rawFile.open('GET', galaxydata, false);
            ...
            rawFile.open('GET', galaxyname, false);
            rawFile.onreadystatechange = function () {
                if (rawFile.readyState === 4) {
                    if (rawFile.status === 200 || rawFile.status == 0) {
...
// Render Loop
            ...
            let centerdist = this.querySelector('#centerdist');
            let centerdistval = 200;
            let startstring = this.querySelector('#startstring');
            ...
            this.querySelector('#srchkey').addEventListener('keyup', function (event) {
                event.preventDefault();
                if (event.keyCode === 13) this.querySelector('#srchbtn').click();
            });

            function searchfunc() {
                let srchkey = this.querySelector('#srchkey').value;
                if (srchkey == '') return;
            ...
            let centerdistly = this.querySelector('#centerdistly');
            let galaxyName = this.querySelector('#galaxyName');
            let galaxyName2 = this.querySelector('#galaxyName2');
            let galaxyNames = this.querySelector('#galaxyNames');
            let galaxyDist = this.querySelector('#galaxyDist');

            let galaxyColor = this.querySelector('#galaxyColor');
            let image = this.querySelector('#image');

            let cdsLink = this.querySelector('#cds');
            let simbad = this.querySelector('#simbad');
            let aladinLink = this.querySelector('#aladin');
            let ned = this.querySelector('#ned');

            let aladinDiv = this.querySelector('#aladin-lite-div');
